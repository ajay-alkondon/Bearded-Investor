...
if ((defined('TESTS_RUNNING') && TESTS_RUNNING) || is_plugin_active('memberpress/memberpress.php')) {
    ...
    define(__NAMESPACE__ . '\DB_VERSION', 17);
    ...
    function autoloader($class_name)
    {
        ...
        if (0 === strpos($class_name, __NAMESPACE__)) {
            ...
            } elseif (0 === strpos($class_name, LIB_NAMESPACE . '\Validatable/')) {
                $filepath = LIB_PATH . "/{$file_name}.php";
            } elseif (0 === strpos($class_name, LIB_NAMESPACE . '\Base/')) {
                $filepath = LIB_PATH . "/{$file_name}.php";
            } elseif (0 === strpos($class_name, ADMIN_CTRLS_NAMESPACE)) {
                $filepath = ADMIN_CTRLS_PATH . "/{$file_name}.php";
            } elseif (0 === strpos($class_name, EMAILS_NAMESPACE)) {
                $filepath = EMAILS_PATH . "/{$file_name}.php";
            } elseif (0 === strpos($class_name, JOBS_NAMESPACE)) {
                $filepath = JOBS_PATH . "/{$file_name}.php";
            } elseif (0 === strpos($class_name, CTRLS_NAMESPACE)) {
                $filepath = CTRLS_PATH . "/{$file_name}.php";
            } elseif (0 === strpos($class_name, HELPERS_NAMESPACE)) {
                $filepath = HELPERS_PATH . "/{$file_name}.php";
            } elseif (0 === strpos($class_name, MODELS_NAMESPACE)) {
                $filepath = MODELS_PATH . "/{$file_name}.php";
            } elseif (0 === strpos($class_name, LIB_NAMESPACE)) {
                $filepath = LIB_PATH . "/{$file_name}.php";
    ...
    // Flush rewrite rules on the next request after activation.
    register_activation_hook(PLUGIN_SLUG, function () {
