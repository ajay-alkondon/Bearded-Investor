jQuery(document).ready(function($) {
  ...
  var SetPage = function (hash) {
    ...
    $('.page').hide();
    $(page).show();
    var mpcs_option_form = $('#mpcs_options_form');
    var raw_url = mpcs_option_form.data('raw-url');
    $('#mpcs_options_form').attr('action', raw_url + '&courses-tab=' + $(page).attr('id'));
  ...
  if(window.URLSearchParams) {
    // Jump to specific tab on load, from URL param
    var params = new URLSearchParams(window.location.search);

    if(params.has('courses-tab')) {
      hash = '#' + params.get('courses-tab');
    }
  }
  ...
  if( MPCS_Settings.plupload_loaded ) {
  ...
  var logoPreviewImg = $("#mpcs-options-logo-preview img");
  if(logoPreviewImg.length && logoPreviewImg.attr("src").length){
    toggle_logo_preview();
  }
