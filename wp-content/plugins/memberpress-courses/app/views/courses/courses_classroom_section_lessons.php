...
<?php foreach ($sections as $section) : ?>
  <div id="section<?php echo ((int) $section->section_order + 1); ?>" class="mpcs-section">
    <button
      id="mpcs-section-header-<?php echo esc_attr($section->id); ?>"
      class="mpcs-section-header"
      aria-expanded="false"
      aria-controls="mpcs-lessons-<?php echo esc_attr($section->id); ?>"
    >
    ...
    <ul
      id="mpcs-lessons-<?php echo esc_attr($section->id); ?>"
      class="mpcs-lessons"
      aria-labelledby="mpcs-section-header-<?php echo esc_attr($section->id); ?>"
    >
      <?php foreach ($section->lessons(true, false) as $lesson_index => $lesson) : ?>
          ...
          <li id="mpcs-lesson-<?php echo esc_attr($lesson->ID); ?>" class="mpcs-lesson <?php
            if ($has_completed_lesson) {
                echo 'completed ';
            } elseif (!$lesson_available) {
                echo 'locked ';
            }
            if ($lesson_available && $is_sidebar && $lesson->ID == get_the_ID()) {
                echo 'current ';
            }
