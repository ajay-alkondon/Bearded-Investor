    ...
    <?php if (! empty(models\Lesson::get_thumbnail($post))) : ?>
      <figure class="figure">
        <a href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>">
          <img src="<?php echo esc_url(models\Lesson::get_thumbnail($post)) ?>" alt="<?php the_title_attribute(); ?>">
        </a>
      </figure>
    <?php endif; ?>
  ...
  <nav class="mpcs-sidebar-content">
    ...
    if (helpers\Courses::is_a_course($post)) { ?>
        <ul class="section mpcs-sidebar-menu">
          ...
          <?php do_action('mpcs_classroom_sidebar_menu', $course, $post); ?>

          <?php if ($course->has_resources()) { ?>
            <li>
                ...
                <div class="tile-content">
                  <p class="tile-title m-0"><?php esc_html_e('Certificate', 'memberpress-courses') ?>
                    <?php if ($course->certificates_share_link == 'enabled') {
                        ?><i title="<?php esc_attr_e('Copied Shareable Certificate Link', 'memberpress-courses') ?>" class="mpcs-share" data-clipboard-text="<?php echo esc_url($share_link); ?>" onclick="return false;"></i><?php
                    } ?>
                  </p>
                </div>
              </a>
