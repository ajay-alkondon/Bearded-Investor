...
class Options extends lib\BaseCtrl
{
    ...
    {
        add_action('init', [$this, 'initialize_options']);
        add_action('admin_menu', [$this, 'add_sub_menu'], 90);
        add_action('mpcs_admin_general_options', [$this, 'general']);
        add_action('mpcs_admin_pages_slugs_options', [$this, 'page_slugs']);
        add_action('wp_ajax_logo_uploader', [$this, 'dnd_logo_uploader']);
        add_filter('mepr_readylaunch_options_data', [$this,'readylaunch_options_data'], 99);
        add_action('mepr_after_readylaunch_options', [$this,'render_readylaunch_settings'], 99);
        add_action('mepr_after_readylaunch_options_table', [$this,'render_readylaunch_options'], 99);
    }

    /**
     * Initialize options data if missing or invalid.
     */
    public function initialize_options()
    {
        ...
        if (!is_array($options)) {
            update_option('mpcs-options', $this->get_default_options());
        }
    }
    ...
    public function get_default_options()
    {
        ...
        return apply_filters('mpcs_default_options', $defaults);
    }
    ...
    {
        if (\MeprUtils::is_logged_in_and_an_admin()) {
            $errors = \MeprHooks::apply_filters('mpcs-validate-options', [], $_POST);
            if (empty($errors)) {
                $this->store_options();
