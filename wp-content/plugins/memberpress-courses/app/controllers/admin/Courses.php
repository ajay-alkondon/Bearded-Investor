...
class Courses extends lib\BaseCptCtrl
{
    ...
    {
        add_filter('manage_' . models\Course::$cpt . '_posts_columns', [$this, 'set_courses_columns'], 1);
        add_action('manage_' . models\Course::$cpt . '_posts_custom_column', [$this, 'courses_columns'], 10, 2);
        add_filter('default_hidden_columns', [$this, 'hide_courses_columns'], 10, 2);
        add_action('admin_footer-edit.php', [$this, 'categories_tags_buttons']);
        add_action('admin_footer-edit-tags.php', [$this, 'categories_tags_return_to_courses_button']);
        add_action('save_post', [$this, 'save_post_data']);
        add_action('admin_enqueue_scripts', [$this, 'admin_enqueue_scripts']);
        add_action('enqueue_block_editor_assets', [$this, 'enqueue_block_editor_assets']);
        add_filter('user_contactmethods', [$this, 'user_social_links']);
        add_action('wp_ajax_mpcs_reset_course_progress', [$this, 'reset_course_progress']);
        add_filter('mepr_list_table_joins', [$this, 'members_table_joins']);
        add_action('mepr_after_admin_members_heading', [$this, 'maybe_display_members_course_heading']);
    ...
    public function categories_tags_return_to_courses_button()
    {
        ...
        if (empty($_GET['taxonomy']) || ! in_array($_GET['taxonomy'], ['mpcs-course-tags', 'mpcs-course-categories'])) {
            return;
        }

        $new_links = sprintf('<a href="%2$s" class="" style="display: block; margin-top: 10px; text-decoration: none;">%1$s</a>', esc_html__('&larr; Back to Courses', 'memberpress-courses'), add_query_arg([
    ...
    public function categories_tags_buttons()
    {
        ...
        $new_links  = sprintf('<a href="%2$s" class="page-title-action" style="margin-left: 0;">%1$s</a>', esc_html__('Categories', 'memberpress-courses'), add_query_arg([
            'taxonomy'  => 'mpcs-course-categories',
            'post_type' => models\Course::$cpt,
        ], admin_url('edit-tags.php')));
        $new_links .= sprintf('<a href="%2$s" class="page-title-action">%1$s</a>', esc_html__('Tags', 'memberpress-courses'), add_query_arg([
    ...
    public static function save_post_data($post_id)
    {
        ...
        $course->certificates_enable               = (isset($_POST[models\Course::$certificates_enable_str])) ? $_POST[models\Course::$certificates_enable_str] : $course->attrs['certificates_enable']['default'];
        ...
        foreach ($curriculum['sections'] as $uuid => $section_data) {
            ...
            foreach ($section_data['lessonIds'] as $index => $lessonId) {
                ...
                if (!defined('DOING_LESSON_SAVE')) {
                    define('DOING_LESSON_SAVE', true);
                }
    ...
    public function admin_enqueue_scripts()
    {
        ...
        if ($current_screen->post_type === models\Course::$cpt && isset($post->ID)) {
            ...
            \wp_enqueue_script('mpcs-courses-js', base\JS_URL . '/admin-courses.js', ['mpcs-course-editor-js', 'vex-js'], base\VERSION);
            ...
            $course_data = [
                ...
                'dripping_time_intervals'      => json_encode(array_map(function ($time) {
                    return [
                        'label' => $time,
                        'value' => strtolower(str_replace(' ', '', $time)),
                    ];
    ...
    public function register_post_type()
    {
        $this->cpt = (object)[
            ...
            'config' => [
                ...
                'rewrite'              => [
                    'slug'       => helpers\Courses::get_permalink_base(),
                    'with_front' => false,
                ],
                'supports'             => ['title', 'excerpt', 'editor', 'thumbnail', 'author'],
                'taxonomies'           => [],
            ],
        ];
    ...
    {
        add_meta_box(models\Course::$cpt . '-builder', __('Curriculum Builder', 'memberpress-courses'), [$this, 'curriculum_meta_box'], models\Course::$cpt, 'normal', 'high');
        add_meta_box(models\Course::$cpt . '-settings', __('Course Setting', 'memberpress-courses'), [$this, 'course_settings_meta_box'], models\Course::$cpt, 'normal', 'high');
        add_meta_box(models\Course::$cpt . '-resources', __('Resources', 'memberpress-courses'), [$this, 'course_resources_meta_box'], models\Course::$cpt, 'normal', 'high');
        add_meta_box(models\Course::$cpt . '-certificates', __('Course Certificates', 'memberpress-courses'), [$this, 'course_certificates_meta_box'], models\Course::$cpt, 'normal', 'high');
        add_meta_box(models\Course::$cpt . '-custom-template', __('Page Options', 'memberpress-courses'), [$this, 'page_options_meta_box'], models\Course::$cpt, 'side', 'default');
    }
    ...
    {
        $section_lesson = models\Lesson::get_one([
            'wheres' => [
                'ID'         => $lesson_id,
                'section_id' => $section_id,
            ],
    ...
    public function reset_course_progress()
    {
        ...
        try {
            ...
            lib\Utils::exit_with_status(403, json_encode(['error' => $e->getErrorMessage()]));
        }

        // Only Admins can delete other user's progress
        if ($user_id != get_current_user_id() && false == lib\Utils::is_user_admin()) {
            lib\Utils::exit_with_status(403, json_encode(['error' => __('You are not allowed to delete user\'s progress', 'memberpress-courses')]));
        }
        ...
        lib\Utils::exit_with_status(200, json_encode(['message' => __('Progress was deleted for this User and Course', 'memberpress-courses')]));
    }
