...
class Account extends lib\BaseCtrl
{
    ...
    {
        add_action('mepr_account_nav', [$this, 'my_courses_nav']);
        add_action('mepr_account_nav_content', [$this, 'my_courses_list']);
    ...
    public static function my_courses_nav($current_user)
    {
        ...
        $tab   = \apply_filters_deprecated('mepr-account-nav-courses-active-name', ['courses'], '1.4.3', 'mpcs_account_nav_courses_tab');
        $tab   = \apply_filters('mpcs_account_nav_courses_tab', $tab);
        $link  = \apply_filters_deprecated('mepr-account-nav-courses-link', [$account_url . $delim . 'action=courses'], '1.4.3', 'mpcs_account_nav_courses_link');
        $link  = \apply_filters('mpcs_account_nav_courses_link', $link);
        $label = \apply_filters_deprecated('mepr-account-nav-courses-label', [__('Courses', 'memberpress-courses')], '1.4.3', 'mpcs_account_nav_courses_label');
        $label = \apply_filters('mpcs_account_nav_courses_label', $label);

        ob_start();
        ?>
    <li class="mepr-nav-item mepr-courses <?php \MeprAccountHelper::active_nav($tab); ?>">
      <a href="<?php echo \esc_url($link); ?>" id="mepr-account-courses">
     ...
     */
    public static function my_courses_list($action, $show_bookmark = true, $attributes = [])
    {
        global $post, $paged;
