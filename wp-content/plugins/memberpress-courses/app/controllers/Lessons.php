...
class Lessons extends lib\BaseCtrl
{
    ...
    {
        add_filter('the_content', [$this, 'prepend_breadcrumbs']);
        add_filter('the_content', [$this, 'append_lesson_navigation']);
        add_filter('the_content', [$this, 'lesson_locked_message'], 99);
        ...
        add_action('wp_ajax_mpcs_record_lesson_progress', [$this, 'record_lesson_progress']);
        add_filter('embed_oembed_html', [$this, 'wrap_oembed_html'], 99, 2);
    ...
    public static function append_lesson_navigation($content)
    {
        ...
        if ($post instanceof \WP_Post && is_single() && in_array($post->post_type, models\Lesson::lesson_cpts(), true)) {
            ...
            $is_lesson_or_logged_out_quiz       = $post->post_type == models\Lesson::$cpt || apply_filters('mpcs_check_quiz_guest_view', false, $post);
            $is_lesson_or_logged_out_assignment = $post->post_type == models\Lesson::$cpt || apply_filters('mpcs_check_assignment_guest_view', false, $post);

            if ($show_buttons && ($is_lesson_or_logged_out_quiz || $is_lesson_or_logged_out_assignment)) {
    ...
    public static function enqueue_scripts()
    {
        ...
        if (is_a($post, 'WP_Post') && is_single() && in_array($post->post_type, models\Lesson::lesson_cpts())) {
            ...
            if (!helpers\App::is_classroom()) {
                ...
                \wp_enqueue_style('mpcs-lesson-css', base\CSS_URL . '/lesson.css', [], base\VERSION);
                \wp_enqueue_script('mpcs-lesson', base\JS_URL . '/lesson.js', ['jquery'], base\VERSION);
    ...
    public static function record_lesson_progress()
    {
        ...
        if (!is_user_logged_in()) {
            lib\Utils::exit_with_status(403, json_encode(['error' => __('Forbidden', 'memberpress-courses')]));
        }

        if (\MeprRule::is_locked(get_post($_POST['lesson_id']))) {
            lib\Utils::exit_with_status(403, json_encode(['error' => __('Unauthorized', 'memberpress-courses')]));
        }
        ...
        try {
            ...
            lib\Utils::exit_with_status(403, json_encode(['error' => $e->getMessage()]));
        }
        ...
        if (!empty($user_progress) && !empty($user_progress->id)) {
            lib\Utils::exit_with_status(403, json_encode(['error' => __('This lesson has already been completed', 'memberpress-courses')]));
        }

        $lesson->complete($current_user->ID);

        lib\Utils::exit_with_status(200, json_encode(['message' => __('Progress was recorded for this User and Lesson', 'memberpress-courses')]));
    }
    ...
    public function wrap_oembed_html($html, $url)
    {
        ...
        $providers = ['vimeo.com', 'youtube.com', 'youtu.be', 'wistia.com', 'wistia.net'];
