...
class CourseProgress extends lib\BaseCtrl
{
    public function load_hooks()
    {
        ...
        add_action('mpca-sub-accounts-links', [$this, 'mpca_sub_account_links'], 20, 3);
        add_action('wp_footer', [$this, 'mpca_sub_account_wp_footer'], 20, 3);
        add_action('wp_ajax_nopriv_mpcs_ca_view_course_progress', [$this, 'mpcs_ca_view_course_progress'], 20, 3);
        add_action('wp_ajax_mpcs_ca_view_course_progress', [$this, 'mpcs_ca_view_course_progress'], 20, 3);
        if (isset($_GET['course'])) {
            add_filter('mepr_admin_members_cols', [$this, 'customize_admin_members_cols']);
            add_filter('mepr_members_list_table_row', [$this, 'customize_admin_members_table_content'], 10, 4);
        }
    }

    function mpca_sub_account_links($mepr_current_user, $ca, $sa)
    {
        echo '<button type="button" data-ca="' . $ca->id . '" data-sa="' . $sa->ID . '" class="mpca-course-sub-account-progress mpca-sub-accounts-button mpca-sub-accounts-button-light">' . __('Course Progress', 'memberpress-courses') . '</button>';
    }
    ...
    public function enqueue_scripts()
    {
        if (isset($_GET['ca']) && isset($_GET['action']) && $_GET['action'] == 'manage_sub_accounts') {
            ...
            \wp_enqueue_style('ca-course-progress', base\CSS_URL . '/ca-course-progress.css', [], base\VERSION);
            \wp_enqueue_script('ca-course-progress-js', base\JS_URL . '/ca-course-progress.js', ['jquery'], base\VERSION, true);
            \wp_localize_script(
                'ca-course-progress-js',
                'mpca_progress',
                [
    ...
    {
        if (isset($_GET['ca']) && isset($_GET['action']) && $_GET['action'] == 'manage_sub_accounts') {
            echo'<div id="course-progress-modal" class="course-progress-modal"><div class="course-progress-modal-content"><button class="course-progress-close-button" aria-label="' . __('Close Course Progress', 'memberpress-courses') . '"><img src="' . base\IMAGES_URL . '/icon-close.png" alt="x"></button><span id="mpca-subaccount-progress"></div></div>';
        }
    }
