...
class App extends lib\BaseCtrl
{
    public function load_hooks()
    {
        ...
        add_action('init', [$this, 'add_comments_support']);
        add_action('admin_notices', [$this, 'courses_activated_admin_notice']);
        add_action('admin_notices', [$this, 'required_wordpress_admin_notice']);
        add_action('admin_init', [$this,'install']); // DB upgrade is handled automatically here now
        add_action('mepr_process_options', [$this,'store_options']);
        add_action('admin_enqueue_scripts', [$this, 'enqueue_admin_scripts']);
        add_action('in_admin_header', [$this, 'admin_header'], 0);
        add_filter('mepr_extend_rules', [$this, 'protect_sections_lessons'], 10, 3);
        add_filter('mepr_content_locked_for_user', [$this, 'protect_courses_lessons'], 10, 4);
        add_action('template_redirect', [$this, 'redirect_to_sales_page'], 1);
        add_action('customize_register', [$this, 'register_customizer']);
        add_filter('post_type_link', [$this, 'lesson_permalink_replace'], 1, 2);
        add_filter('rewrite_rules_array', [$this, 'lesson_permalink_rules']);
        add_filter('use_block_editor_for_post_type', [$this, 'force_block_editor_for_courses'], 999, 2);
        add_filter('mepr_pre_run_rule_content', [$this, 'show_more_content_on_archive_page'], 10, 3);
        add_filter('the_title', [$this, 'show_lock_icon']);
        add_action('plugins_loaded', [$this, 'addon_updates']);
        add_filter('mepr_view_paths', [$this, 'add_view_path']);
        add_filter('mepr_rule_types_before_partial', [$this, 'course_section_rule_type']);
        add_filter('mepr_rule_contents_array', [$this, 'course_section_contents_array'], 10, 2);
        add_filter('mepr_rule_content', [$this, 'course_section_content'], 10, 3);
        add_filter('mepr_rule_has_content', [$this, 'has_course_section'], 10, 2);
        add_filter('mepr_rule_search_content', [$this, 'find_course_sections'], 10, 3);
        add_filter('mepr_extend_post_rules', [$this, 'extend_post_rules'], 10, 3);
        add_filter('mepr_rule_access_types', [$this, 'add_rule_access_types']);
        add_filter('mepr_rules_js_access_row', [$this, 'add_rule_access_rows']);
        add_filter('mepr_rule_access_condition_field_html', [$this, 'course_completed_access_condition_field'], 10, 5);
        add_filter('mepr_rule_is_post_locked', [$this, 'is_content_locked_until_course_completed'], 10, 2);
        add_filter('mepr_rule_is_uri_locked', [$this, 'is_content_locked_until_course_completed'], 10, 2);
        add_filter('mepr_pre_run_partial_rule', [$this, 'is_shortcode_content_protected_until_course_completed'], 10, 3);
        add_filter('comments_template', [$this, 'course_comments_template']);
        add_action(base\SLUG_KEY . '_courses_footer', [$this, 'do_wordpress_footer']);
        add_action('wp_footer', [$this, 'record_course_events']);
        add_filter('mpdt-member-extend-obj-data', ['memberpress\courses\helpers\Events', 'mpdt_member_extend_object'], 10, 2);
        add_filter('mdpt-ajax-event-data', ['memberpress\courses\helpers\Events', 'mpdt_event_data']);
        add_filter('mpdt-sample-event-data', ['memberpress\courses\helpers\Events', 'mpdt_event_data']);
        add_action('menu_order', [$this, 'admin_menu_order'], 11);
        add_action('custom_menu_order', [$this, 'admin_menu_order'], 11);
        add_action('plugins_loaded', [$this, 'autoinstall_quizzes_addon']);
        add_action('wp_ajax_mpcs_search_courses', [$this, 'ajax_search_courses']);
    }

    public function ajax_search_courses()
    {
        check_ajax_referer('mpcs_search_courses', 'security');
    ...
    public function store_options()
    {
        if (lib\Utils::is_post_request() && isset($_POST['mpcs-options']) && is_array($_POST['mpcs-options'])) {
            ...
            $old_options          = get_option('mpcs-options', []);
    ...
    {
        $app = App::fetch();
        add_action('admin_menu', [$app,'menu'], 20);
    }
    ...
    public static function admin_separator()
    {
        ...
        $menu[] = ['', 'read', 'separator-' . base\PLUGIN_NAME, '', 'wp-menu-separator ' . base\PLUGIN_NAME];
    }
    ...
    {
        \wp_enqueue_style('mpcs-simplegrid', base\CSS_URL . '/vendor/simplegrid.css', [], base\VERSION);
        \wp_enqueue_style('mpcs-fontello-styles', base\FONTS_URL . '/fontello/css/mp-courses.css', [], base\VERSION);
        \wp_enqueue_style('mpcs-admin-shared', base\CSS_URL . '/admin_shared.css', ['wp-pointer','mpcs-simplegrid','mpcs-fontello-styles'], base\VERSION);
        \wp_enqueue_script('mpcs-tooltip', base\JS_URL . '/tooltip.js', ['jquery','wp-pointer'], base\VERSION);
        if (strstr($hook, 'memberpress-options') !== false || strstr($hook, 'memberpress-courses-options') !== false || strstr($hook, 'mp-courses') !== false) {
            \wp_enqueue_style('mpcs-settings-table', base\CSS_URL . '/settings_table.css', [], base\VERSION);
            \wp_enqueue_script('mpcs-settings-table', base\JS_URL . '/settings_table.js', ['jquery', 'wp-color-picker'], base\VERSION);
            ...
            $plupload_init = [
                ...
                'filters'             => [
                    [
                        'title'      => __('Allowed Files', 'memberpress-courses'),
                        'extensions' => '*',
                    ],
                ],
                'multipart'           => true,
     ...
     *
     * @see    load_hooks(), add_filter('mepr_extend_rules')
     ...
     *
     * @param  boolean  $bool    whether to lock the content or not
    ...
    public function register_customizer($wp_customize)
    {
        ...
        $sections = apply_filters(base\SLUG_KEY . '_customiser_sections', []);
        $settings = apply_filters(base\SLUG_KEY . '_customiser_settings', []);
    ...
    {
        $customRules = [];
        $customRules[ helpers\Courses::get_permalink_base() . '/([^/]+)/' . helpers\Lessons::get_permalink_base() . '/([^/]+)/?$' ] = 'index.php?post_type=' . models\Lesson::$cpt . '&name=$matches[2]'; // resolve lesson deterministically
    ...
    public function show_lock_icon($title)
    {
        ...
        if (strpos($title, 'mpcs-lock') !== false) {
            return $title;
        }
    ...
    function course_section_rule_type($all_types)
    {
        return array_merge(
            $all_types,
            ['course_section' => __('Course Section', 'memberpress-courses')]
        );
    }
    ...
    public function course_comments_template($template)
    {
        ...
        if (!in_array($post->post_type, ['mpcs-course', 'mpcs-lesson'])) {
            return $template;
        }
        $options              = \get_option('mpcs-options');
        $show_course_comments = helpers\Options::val($options, 'show-course-comments');
          ...
          <article id="comment-<?php comment_ID(); ?>" class="course-comment">
            <div class= 'course-comment-info'>
              ...
              <div class="course-comment-author vcard">
                ...
                printf(
                    __('<div class="course-comment-time"><span class="timendate"><a href="%1$s"><time datetime="%2$s">%3$s</time></a></span></div>', 'memberpress-courses'),
    ...
    /**
     * Adds additional access types to MemberPress rules.
     *
     * @param array $access_types The existing access types.
     */
    public function add_rule_access_types(array $access_types): array
    {
        $access_types[] = [
            'label' => __('Course Completed', 'memberpress-courses'),
    ...
    /**
     * Adds additional access rows to the 'mepr-rules-js' file.
     *
     * @param array $rows The existing access rows.
     */
    public function add_rule_access_rows(array $rows): array
    {
        $rows['course_completed'] = [
            'row_tpl'       => \MeprRulesHelper::access_row_string(new \MeprRuleAccessCondition(['access_type' => 'course_completed']), 1),
            'types_tpl'     => \MeprRulesHelper::access_types_dropdown_string('course_completed'),
            'operator_tpl'  => \MeprRulesHelper::access_operators_dropdown_string('course_completed'),
            'condition_tpl' => \MeprRulesHelper::access_conditions_field_string('course_completed'),
        ];
    ...
    /**
     * Returns the HTML for the course completed access condition field.
     *
     * @param string $html          Default HTML for access condition fields.
     * @param string $type          Access condition type.
     * @param mixed  $value         Current value of the field
     * @param string $field_name    Name of the field.
     * @param string $field_classes Additional HTML classes for the field.
     */
    public function course_completed_access_condition_field(string $html, string $type, $value, string $field_name, string $field_classes): string
    {
        if ('course_completed' === $type) {
            return \MeprView::get_string('/admin/rules/courses_input', compact('field_name', 'field_classes', 'value'));
        }
    ...
    /**
     * Checks if content is locked until a course is completed based on the given rules.
     *
     * @param boolean|null $is_locked Whether the content is locked.
     * @param \MeprRule[]  $rules     The rules associated with the content.
     */
    public function is_content_locked_until_course_completed(?bool $is_locked, array $rules): ?bool
    {
        $is_locked_for_current_user = $this->is_content_locked_for_current_user_until_course_completed($rules);

        return null !== $is_locked_for_current_user
        ? $is_locked_for_current_user
        : $is_locked;
    }

    /**
     * Checks if shortcode content is allowed.
     *
     * @param boolean $is_allowed      Whether the shortcode content is allowed.
     * @param boolean $hide_if_allowed Whether to hide the content if allowed.
     * @param array   $atts            Shortcode attributes.
     */
    public function is_shortcode_content_protected_until_course_completed($is_allowed, $hide_if_allowed, $atts): bool
    {
        $rule_ids = [];
    ...
    /**
     * Checks if content is locked for the current user based on the course_completed rules.
     *
     * @param \MeprRule[] $rules The rules associated with the content.
     *
     * @return boolean|null Returns true if content is locked, false if not, or null if no course completion rule is found.
     */
    private function is_content_locked_for_current_user_until_course_completed(array $rules): ?bool
    {
        foreach ($rules as $rule) {
            foreach ($rule->access_conditions() as $condition) {
                if ('course_completed' === $condition->access_type) {
                    $current_user_id = \MeprUtils::get_current_user_id();
                    $course_id       = (int) $condition->access_condition;
                    $completed       = models\UserProgress::has_completed_course($current_user_id, $course_id);
    ...
    public function autoinstall_quizzes_addon()
    {
        ...
        try {
            ...
            $plugin_url = esc_url_raw(add_query_arg(['page' => 'memberpress-addons'], admin_url('admin.php')));
