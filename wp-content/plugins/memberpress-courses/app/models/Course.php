...
class Course extends lib\BaseCptModel
{
    ...
    public static $dripping_str                          = 'mpcs-dripping';
    public static $drip_type_str                         = 'mpcs-drip-type';
    public static $drip_amount_str                       = 'mpcs-drip-amount';
    public static $drip_time_str                         = 'mpcs-drip-time';
    public static $drip_timezone_str                     = 'mpcs-drip-timezone';
    public static $drip_frequency_str                    = 'mpcs-drip-frequency';
    public static $drip_frequency_type_str               = 'mpcs-drip-frequency-type';
    public static $drip_frequency_fixed_date_str         = 'mpcs-drip-frequency-fixed-date';
    public static $drip_lessons_str                      = 'mpcs-drip-item-lessons';
    public static $drip_quizzes_str                      = 'mpcs-drip-item-quizzes';
    public static $drip_assignments_str                  = 'mpcs-drip-item-assignments';
    public static $not_dripped_message_str               = 'mpcs-not-dripped-message';
    ...
    public function __construct($obj = null)
    {
        ...
        $this->load_cpt(
            ...
            [
                ...
                'certificates_background_color'     => [
                    'default' => '',
                    'type'    => 'string',
                ],
                'certificates_title'                => [
                    'default' => 'This certificate is awarded to',
                    'type'    => 'string',
                ],
                'certificates_footer_message'       => [
                    'default' => 'for successfully completing the course',
                    'type'    => 'string',
                ],
                'certificates_style'                => [
                    'default' => 'style_a',
                ...
                'not_dripped_message'               => [
                    'default' => __('Curriculum is released every {mpcs_drip_schedule}. As such, this {mpcs_item_type} will not be available to you until {mpcs_drip_date}.', 'memberpress-courses'),
                    'type'    => 'string',
                ],
            ]
        );
     ...
     *
     * @param  string       $type            The data type to return: 'objects' or 'ids'.
     * @param  array        $cpts            Optionally filter lessons by post type.
     * @param  boolean      $include_private Whether to include private lessons.
     * @param  boolean|null $include_draft   Whether to include draft lessons.
     * @return array<Lesson|\memberpress\quizzes\models\Quiz>|int[]
     */
    public function lessons($type = 'objects', $cpts = [], $include_private = true, $include_draft = null)
    {
        if ($type === 'ids') {
            return models\Lesson::find_all_ids_by_course((int) $this->ID);
        }

        $lessons     = [];
        $section_ids = models\Section::find_all_ids_by_course((int) $this->ID);

        foreach ($section_ids as $section_id) {
            $lessons = array_merge(
                $lessons,
                models\Lesson::find_all_by_section($section_id, $cpts, $include_private, $include_draft)
            );
        }
     ...
     *
     * @return integer Query count result
     */
    public function number_of_lessons()
    {
        return count($this->lessons('ids'));
    }
     ...
     *
     * @param  integer $user_id User to check progress against
    ...
    public function user_progress($user_id)
    {
        ...
        $total_progress = $total_lessons * 100.00;
      ...
      ',
        ...
        $progress = $completed_progress / $total_progress * 100.00;
    ...
    public function quizzes($type = 'objects')
    {
        ...
        return apply_filters('mpcs_course_quizzes', $quizzes, $this, $sections, $type);
    }

    public function get_resources()
    {
        $resources = ! is_array($this->resources) ? (array) json_decode($this->resources, true) : [];
    ...
    /**
     * Checks whether or not a course has any associated resources.
     *
     * @return boolean True if the course has resources, otherwise false.
     */
    public function has_resources()
    {
        $resources = $this->get_resources();

        return is_array($resources) && !empty($resources['items']);
    }
}
