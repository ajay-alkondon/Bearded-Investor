...
class Events
{
    ...
    public static function mpdt_event_data($mbr)
    {
        ...
        if (false !== $data) {
            $cleanup_keys = apply_filters('mpca-mpdt-user-object-excluded-keys', ['active_memberships', 'first_txn', 'latest_txn', 'profile', 'recent_transactions', 'recent_subscriptions', 'login_count', 'sub_count', 'active_txn_count', 'expired_txn_count', 'trial_txn_count', 'address']);
    ...
    private static function maybe_start_course($user_id, $course_id)
    {
        ...
        if (empty(trim($course_started_meta))) {
            $user = new \MeprUser($user_id);
            \MeprEvent::record('mpca-course-started', $user, ['course_id' => $course_id]);
