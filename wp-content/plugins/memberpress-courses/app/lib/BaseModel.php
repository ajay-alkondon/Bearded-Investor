...
abstract class BaseModel
{
    ...
    public function __get($name)
    {
        ...
        if (method_exists($this, $extend_fn)) {
            $value = call_user_func([$this,$extend_fn], $name, $value);
        }
    ...
    public function load_from_array($values)
    {
        ...
        foreach ($values as $key => $value) {
            ...
            if (isset($attrs[$key])) {
                if (in_array($attrs[$key]['type'], ['bool','boolean'])) {
                    $value = (bool)$value;
                } elseif ($attrs[$key]['type'] == 'float') {
                    $value = (float)$value;
                } elseif ($attrs[$key]['type'] == 'integer') {
                    $value = (int)$value;
                }
            }
     ...
     *
     * @param  array|integer $args Array of args or numeric ID
     * @return static|false
     */
    ...
    public function get_attr_from_key($key)
    {
        ...
        if (
            preg_match('/^_' . base\SLUG_KEY . "_{$model_str}_(.*)$/", $key, $m) &&
