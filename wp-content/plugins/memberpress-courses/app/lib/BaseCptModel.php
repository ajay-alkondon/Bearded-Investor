 ...
 * @property string $post_name The post name (slug)
 * @property string $post_date The post created date
 * @property string $post_status The post status
 * @property int $post_parent The post parent
...
abstract class BaseCptModel extends BaseBuiltinModel
{
    ...
    public function store($validate = true)
    {
        ...
        $store_action = empty($this->ID) ? 'create' : 'update';

        if ($store_action == 'update') {
            $id = \wp_update_post(wp_slash((array)$this->rec));
        } else {
            $id = \wp_insert_post(wp_slash((array)$this->rec));
        }
    ...
    public function store_meta()
    {
        ...
        foreach ($attrs as $attr => $attr_value) {
            ...
            update_post_meta($this->ID, wp_slash($attr_key), wp_slash($this->cast_attr($attr, $attr_value)));
        }
    }
    ...
    public function destroy()
    {
        ...
        if (false === $res) {
            throw new CreateException(sprintf(__('This was unable to be deleted.', 'memberpress-courses')));
        }
     ...
     * @return static|false
     */
    public static function get_one($options = [])
    {
    ...
    protected static function get_all_objects($options = [])
    {
    ...
    protected static function get_count_by_class($class, $args = [])
    {
        global $wpdb;
        $db = Db::fetch();
        ...
        return $db->get_count($wpdb->posts, ['post_type' => $cpt]);
    }
