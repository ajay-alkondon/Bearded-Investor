...
abstract class CtaxModel extends BuiltinModel
{
    ...
    public function store($validate = true)
    {
        ...
        if ($validate) {
            try {
                ...
                return new WP_Error(
                    \get_class($e),
                    sprintf(
                        __('%1$s was unable to be saved: %2$s', 'memberpress-courses'),
                        \get_class($this),
                        $e->getMessage()
                    )
                );
        ...
        if (\is_wp_error($id)) {
            ...
            if (is_numeric($id)) {
                $this->term_id = $id;
            } elseif (is_array($id) && isset($id['term_id'])) {
    ...
    protected static function get_one($args)
    {
        ...
        if (is_numeric($args)) {
            $args = [$wpdb->prepare('t.term_id=%d', $args)];
        }
    ...
    protected static function get_all($order_by = '', $limit = '', $args = [])
    {
        $class = get_called_class();
    ...
    protected static function get_count($args = [])
    {
        global $wpdb;
        $db = Db::fetch();
        ...
        return $db->get_count($wpdb->terms, ['taxonomy' => $ctax]);
    }
