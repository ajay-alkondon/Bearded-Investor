  ...
  <header class="clearfix">
    ...
    <?php else: ?>
      ...
      <?php else: ?>
       <h1>
            <?php
              $heading = esc_html__( 'INVOICE NO', 'memberpress-pdf-invoice' );
              if ($is_offline_renewal) {
                $heading = esc_html__( 'PROFORMA INVOICE NO', 'memberpress-pdf-invoice' );
              }
              printf( '%s: %s', $heading, strtoupper( $invoice->invoice_number ) );
            ?>
        </h1>
      <?php endif; ?>
    <?php endif; ?>
    ...
    <table>
      <tr>
        <td id="project">
         <?php
          if ($is_offline_renewal) {
            if ( isset($invoice->invoice_issue_date_ts) ) {
              echo '<br/> ' . esc_html__( 'Issue Date', 'memberpress-pdf-invoice' ) . ': ' . date_i18n( get_option( 'date_format' ), $invoice->invoice_issue_date_ts );
            }
            if ( isset($invoice->invoice_due_date_ts) ) {
              echo '<br/> ' . esc_html__( 'Due Date', 'memberpress-pdf-invoice' ) . ': ' . date_i18n( get_option( 'date_format' ), $invoice->invoice_due_date_ts ) . ' <br/><br/> ';
            }
          }
          ?>
    ...
    <table>
      <tr>
        ...
        <td>
          ...
          <?php else: ?>
            <img class="img-responsivel" style="width:150px; float:right" src="<?php echo esc_attr($invoice->paid_logo_url); ?>" alt="<?php echo esc_attr($invoice->payment_status); ?>-stamp">
          <?php endif; ?>
        </td>
      </tr>
    </table>
