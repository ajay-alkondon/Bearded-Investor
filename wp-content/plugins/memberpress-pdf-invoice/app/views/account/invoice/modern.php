...
// Is Offline and the transaction is pending.
$is_offline_renewal = MePdfInvoicesHelper::is_offline_renewal_transaction($txn) && $invoice->payment_status == \MeprTransaction::$pending_str;
?>
<!DOCTYPE html>
...
<body>
    ...
    <main>
        ...
        <div class="container">
            <table id="desc">
                <tr>
                    ...
                    <td>
                        ...
                        <?php endif; ?>
                        <?php if ($is_offline_renewal) : ?>
                            <?php if ( isset($invoice->invoice_issue_date_ts) ) : ?>
                            <p><?php echo esc_html__('Issue Date', 'memberpress-pdf-invoice'); ?>
                                <?php echo date_i18n(get_option('date_format'), $invoice->invoice_issue_date_ts); ?></p>
                            <?php endif; ?>
                            <?php if ( isset($invoice->invoice_due_date_ts) ) : ?>
                            <p><?php echo esc_html__('Due Date', 'memberpress-pdf-invoice'); ?>
                                <?php echo date_i18n(get_option('date_format'), $invoice->invoice_due_date_ts); ?></p>
                            <?php endif; ?>
                        <?php else: ?>
            ...
            <?php elseif ($is_offline_renewal) : ?>
                <h1 id="header"><?php esc_html_e('Proforma Invoice', 'memberpress-pdf-invoice') ?></h1>
            <?php else : ?>
                ...
                <tbody>
                    ...
                    foreach ($invoice->items as $item) {
                        ...
                        <tr>
                            ...
                            <td class="mp-currency-cell">
                                <?php echo MeprAppHelper::format_currency($item['amount'], true, false); ?></td>
                        </tr>
                        <?php
                    }
                    ?>
                    ...
                    <?php if (is_array($invoice->tax_items) && count($invoice->tax_items)) : ?>
                        <tr>
                            ...
                            <?php endif; ?>
                            <td class="total">
                                <?php echo MeprAppHelper::format_currency($invoice->subtotal, true, false); ?></td>
                        </tr>
                            ...
                            <?php if ($tax_item['amount'] > 0 || $tax_item['percent'] > 0) : ?>
                                <tr>
                                    <?php if ($invoice->show_quantity) : ?>
                                        <td>&nbsp;</td>
                                    <?php endif; ?>
                                    <td class="mepr-tax-invoice">
                                        <?php echo MeprUtils::format_tax_percent_for_display($tax_item['percent']) . '% ' . $tax_item['type']; ?>
                                        <?php if (count($invoice->tax_items) > 1 && isset($tax_item['post_title']) && !empty($tax_item['post_title'])) : ?><br /><small><?php echo esc_html($tax_item['post_title']); ?></small><?php endif; ?>
                                    </td>
                                    <td class="mp-currency-cell">
                                        <?php echo MeprAppHelper::format_currency($tax_item['amount'], true, false); ?></td>
                                </tr>
                            <?php endif; ?>
                        <?php endforeach; ?>
                    <?php endif; ?>
                ...
                <tr>
                    ...
                    <td>
                        <?php if ($invoice->payment_status == 'pending') : ?>
                            <img class="img-responsivel" style="width:150px; float:right" src="<?php echo esc_attr($invoice->unpaid_logo_url); ?>" alt="<?php echo esc_attr($invoice->payment_status); ?>-stamp">
                        <?php else : ?>
                            <img class="img-responsivel" style="width:150px; float:right"
                                src="<?php echo esc_attr($invoice->paid_logo_url); ?>"
                                alt="<?php echo esc_attr($invoice->payment_status); ?>-stamp">
                        <?php endif; ?>
                    </td>
                </tr>
            </table>
