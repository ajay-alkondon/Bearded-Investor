...
class MePdfInvoicesHelper {
  ...
  /**
   * Check if automatic renewal invoices is enabled for a MeprArtificialGateway gateway and get days before renewal
   *
   * @param string $gateway_id The gateway ID to check
   * @return boolean True if enabled, false otherwise.
   */
  public static function is_offline_renewal_invoice_enabled($gateway_id) {
  ...
  /**
   *  Determines if a transaction is an offline renewal.
   *
   * @param object $txn Transaction object containing the transaction details.
   * @return bool Returns true if the transaction is an offline renewal, false otherwise.
   */
  public static function is_offline_renewal_transaction($txn) {
    $is_test_renewal_email = MeprHooks::apply_filters('mepr_pdf_invoice_test_renewal_email', false, $txn);

    if ($is_test_renewal_email) {
      // If this is a test renewal email, we assume it is an offline renewal transaction
      return true;
    }

    if (strstr($txn->trans_num,'mp-rtxn-')
      && self::is_offline_renewal_invoice_enabled($txn->gateway)
      && MePdfProformaInvoiceNumber::find_invoice_num_by_txn_id($txn->id) ) {
      return true;
    }
    return false;
  }
