...
class MePdfInvoiceNumber extends MeprBaseModel {
  ...
  public static function find_invoice_num($invoice_num){
    global $wpdb;
    $db = MePdfDB::fetch();
    $query = $wpdb->prepare("SELECT id FROM {$db->invoice_numbers} WHERE invoice_number = %d LIMIT 1", (int) $invoice_num);
  ...
  public static function find_invoice_num_by_txn_id($txn_id){
    global $wpdb;
    $db = MePdfDB::fetch();
    $query = $wpdb->prepare("SELECT invoice_number FROM {$db->invoice_numbers} WHERE transaction_id = %d LIMIT 1", (int) $txn_id);
