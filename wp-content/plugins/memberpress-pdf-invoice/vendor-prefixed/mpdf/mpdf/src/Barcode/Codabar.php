 ...
 */
class Codabar extends \MemberPress\PdfInvoice\Mpdf\Barcode\AbstractBarcode implements \MemberPress\PdfInvoice\Mpdf\Barcode\BarcodeInterface
{
	...
	private function init($code, $printRatio)
	{
		...
		for ($i = 0; $i < $len; ++$i) {
			...
			if (!isset($chr[$code[$i]])) {
				throw new \MemberPress\PdfInvoice\Mpdf\Barcode\BarcodeException(sprintf('Invalid character "%s" in CODABAR barcode value "%s"', $code[$i], $code));
			}
