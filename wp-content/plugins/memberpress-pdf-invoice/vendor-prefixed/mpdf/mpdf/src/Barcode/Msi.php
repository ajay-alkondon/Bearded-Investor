 ...
 */
class Msi extends \MemberPress\PdfInvoice\Mpdf\Barcode\AbstractBarcode implements \MemberPress\PdfInvoice\Mpdf\Barcode\BarcodeInterface
{
	...
	private function init($code, $checksum)
	{
		...
		for ($i = 0; $i < $clen; ++$i) {
			...
			if (!isset($chr[$digit])) {
				// invalid character
				throw new \MemberPress\PdfInvoice\Mpdf\Barcode\BarcodeException(sprintf('Invalid character "%s" in MSI barcode value "%s"', $digit, $code));
			}
			$seq .= $chr[$digit];
		}
