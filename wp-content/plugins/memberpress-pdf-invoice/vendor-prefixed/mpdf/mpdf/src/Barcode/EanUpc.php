 ...
 */
class EanUpc extends \MemberPress\PdfInvoice\Mpdf\Barcode\AbstractBarcode implements \MemberPress\PdfInvoice\Mpdf\Barcode\BarcodeInterface
{
	...
	private function init($code, $length)
	{
		if (preg_match('/[\D]+/', $code)) {
			throw new \MemberPress\PdfInvoice\Mpdf\Barcode\BarcodeException(sprintf('Invalid EAN UPC barcode value "%s"', $code));
		}
		...
		if ($upce) {
			...
			if ($invalidUpce) {
				throw new \MemberPress\PdfInvoice\Mpdf\Barcode\BarcodeException('UPC-A cannot produce a valid UPC-E barcode');
			}
		}
