 ...
 */
class I25 extends \MemberPress\PdfInvoice\Mpdf\Barcode\AbstractBarcode implements \MemberPress\PdfInvoice\Mpdf\Barcode\BarcodeInterface
{
	...
	private function init($code, $printRatio, $checksum)
	{
		...
		for ($i = 0; $i < $clen; $i = ($i + 2)) {
			...
			if ((!isset($chr[$charBar])) or (!isset($chr[$charSpace]))) {
				// invalid character
				throw new \MemberPress\PdfInvoice\Mpdf\Barcode\BarcodeException(sprintf('Invalid I25 barcode value "%s"', $code));
			}
