 ...
 */
class Imb extends \MemberPress\PdfInvoice\Mpdf\Barcode\AbstractBarcode implements \MemberPress\PdfInvoice\Mpdf\Barcode\BarcodeInterface
{
	...
	public function __construct($code, $xDim, $gapWidth, $daft)
	{
		if (!function_exists('bcadd')) {
			throw new \MemberPress\PdfInvoice\Mpdf\Barcode\BarcodeException('IMB barcodes require bcmath extension to be loaded.');
		}
	...
	private function init($code, $gapWidth, $daft)
	{
		...
		switch (strlen($routingCode)) {
			...
			case 11:
				...
				break;
			default:
				throw new \MemberPress\PdfInvoice\Mpdf\Barcode\BarcodeException(sprintf('Invalid MSI routing code "%s"', $routingCode));
		}
