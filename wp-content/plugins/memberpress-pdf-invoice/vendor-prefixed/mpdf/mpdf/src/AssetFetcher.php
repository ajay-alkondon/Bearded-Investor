...
use MemberPress\PdfInvoice\Mpdf\File\LocalContentLoaderInterface;
use MemberPress\PdfInvoice\Mpdf\File\StreamWrapperChecker;
use MemberPress\PdfInvoice\Mpdf\Http\ClientInterface;
use MemberPress\PdfInvoice\Mpdf\Log\Context as LogContext;
use MemberPress\PdfInvoice\Mpdf\PsrHttpMessageShim\Request;
use MemberPress\PdfInvoice\Mpdf\PsrLogAwareTrait\PsrLogAwareTrait;
use MemberPress\PdfInvoice\Psr\Log\LoggerInterface;

class AssetFetcher implements \MemberPress\PdfInvoice\Psr\Log\LoggerAwareInterface
{
	...
	public function fetchDataFromPath($path, $originalSrc = null)
	{
		...
		if ($wrapperChecker->hasBlacklistedStreamWrapper($path)) {
			throw new \MemberPress\PdfInvoice\Mpdf\Exception\AssetFetchingException('File contains an invalid stream. Only ' . implode(', ', $wrapperChecker->getWhitelistedStreamWrappers()) . ' streams are allowed.');
		}
	...
	public function fetchRemoteContent($path)
	{
		...
		try {
			...
			/** @var \MemberPress\PdfInvoice\Mpdf\PsrHttpMessageShim\Response $response */
			...
			if (!str_starts_with((string) $response->getStatusCode(), '2')) {
				...
				if ($this->mpdf->debug) {
					throw new \MemberPress\PdfInvoice\Mpdf\MpdfException($message);
				}
			...
			if ($this->mpdf->debug) {
				throw new \MemberPress\PdfInvoice\Mpdf\MpdfException($message, 0, E_ERROR, null, null, $e);
			}
