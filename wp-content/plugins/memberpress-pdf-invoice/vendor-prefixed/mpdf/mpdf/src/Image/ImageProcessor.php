...
namespace MemberPress\PdfInvoice\Mpdf\Image;

use MemberPress\PdfInvoice\Mpdf\AssetFetcher;
...
use MemberPress\PdfInvoice\Mpdf\Gif\Gif;
use MemberPress\PdfInvoice\Mpdf\Language\LanguageToFontInterface;
use MemberPress\PdfInvoice\Mpdf\Language\ScriptToLanguageInterface;
...
use MemberPress\PdfInvoice\Mpdf\PsrLogAwareTrait\PsrLogAwareTrait;
use MemberPress\PdfInvoice\Mpdf\SizeConverter;
use MemberPress\PdfInvoice\Psr\Log\LoggerInterface;

class ImageProcessor implements \MemberPress\PdfInvoice\Psr\Log\LoggerAwareInterface
{
	...
	/**
	 * @var \MemberPress\PdfInvoice\Mpdf\Image\ImageTypeGuesser
	 */
	private $guesser;
	...
	public function getImage(&$file, $firstTime = true, $allowvector = true, $orig_srcpath = false, $interpolation = false)
	{
		...
		if (!$data) {
			...
			} catch (\MemberPress\PdfInvoice\Mpdf\Exception\AssetFetchingException $e) {
	...
	private function gzCompress($data)
	{
		if (!function_exists('gzcompress')) {
			throw new \MemberPress\PdfInvoice\Mpdf\MpdfException('gzcompress is not available. install ext-zlib extension.');
		}
	...
	private function imageError($file, $firstTime, $msg)
	{
		...
		if ($firstTime && ($this->mpdf->showImageErrors || $this->mpdf->debug)) {
			throw new \MemberPress\PdfInvoice\Mpdf\MpdfImageException(sprintf('%s (%s)', $msg, substr($file, 0, 256)));
		}
	...
	public function processJpg($data, $file, $firstTime, $interpolation)
	{
		...
		if ($a[2] === 'DeviceCMYK' && ($this->mpdf->restrictColorSpace === 2 || ($this->mpdf->PDFA && $this->mpdf->restrictColorSpace !== 3))) {
			...
			if (!function_exists('gd_info')) {
				throw new \MemberPress\PdfInvoice\Mpdf\MpdfException(sprintf('JPG image may not use CMYK color space (%s).', $file));
			}
	...
	public function processPng($data, $file, $firstTime, $interpolation)
	{
		...
		if ($firstTime && ($colspace === 'DeviceRGB' || $colspace === 'Indexed') && ($this->mpdf->PDFX || $this->mpdf->restrictColorSpace === 3)) {
			...
			if ($pngalpha) {
				if ($this->mpdf->PDFA) {
					throw new \MemberPress\PdfInvoice\Mpdf\MpdfException(sprintf('PDFA1-b does not permit images with alpha channel transparency (%s).', $file));
				}
