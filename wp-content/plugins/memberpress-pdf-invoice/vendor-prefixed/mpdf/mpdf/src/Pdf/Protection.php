...
namespace MemberPress\PdfInvoice\Mpdf\Pdf;
...
class Protection
{
	...
	/**
	 * @var \MemberPress\PdfInvoice\Mpdf\Pdf\Protection\UniqidGenerator
	 */
	...
	public function setProtection($permissions = [], $user_pass = '', $owner_pass = null, $length = 40)
	{
		...
		if ($length === 128) {
			...
			throw new \MemberPress\PdfInvoice\Mpdf\MpdfException('PDF protection only allows lenghts of 40 or 128');
		}
	...
	private function getProtectionBitsFromOptions($permissions)
	{
		...
		foreach ($permissions as $permission) {
			if (!isset($this->options[$permission])) {
				throw new \MemberPress\PdfInvoice\Mpdf\MpdfException(sprintf('Invalid permission type "%s"', $permission));
			}
