...
class DirectWrite
{
	...
	function CircularText($x, $y, $r, $text, $align = 'top', $fontfamily = '', $fontsizePt = 0, $fontstyle = '', $kerning = 120, $fontwidth = 100, $divider = '')
	{
		...
		if ($kerning == 0) {
			throw new \MemberPress\PdfInvoice\Mpdf\MpdfException('Please use values unequal to zero for kerning (CircularText)');
		}

		if ($fontwidth == 0) {
			throw new \MemberPress\PdfInvoice\Mpdf\MpdfException('Please use values unequal to zero for font width (CircularText)');
		}
