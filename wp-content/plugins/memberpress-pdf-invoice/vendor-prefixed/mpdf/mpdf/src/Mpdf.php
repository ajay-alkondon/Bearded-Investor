...
use MemberPress\PdfInvoice\Mpdf\Config\ConfigVariables;
use MemberPress\PdfInvoice\Mpdf\Config\FontVariables;
use MemberPress\PdfInvoice\Mpdf\Conversion;
use MemberPress\PdfInvoice\Mpdf\Css\Border;
use MemberPress\PdfInvoice\Mpdf\Css\TextVars;
use MemberPress\PdfInvoice\Mpdf\Log\Context as LogContext;
use MemberPress\PdfInvoice\Mpdf\Fonts\MetricsGenerator;
use MemberPress\PdfInvoice\Mpdf\Output\Destination;
use MemberPress\PdfInvoice\Mpdf\PsrLogAwareTrait\MpdfPsrLogAwareTrait;
use MemberPress\PdfInvoice\Mpdf\QrCode;
use MemberPress\PdfInvoice\Mpdf\Utils\Arrays;
use MemberPress\PdfInvoice\Mpdf\Utils\NumericString;
use MemberPress\PdfInvoice\Mpdf\Utils\UtfString;
use MemberPress\PdfInvoice\Psr\Log\NullLogger;
 ...
 * @license GPL-2.0
 */
class Mpdf implements \MemberPress\PdfInvoice\Psr\Log\LoggerAwareInterface
{
	...
	/**
	 * @var \MemberPress\PdfInvoice\Mpdf\Gradient
	 */
	private $gradient;

	/**
	 * @var \MemberPress\PdfInvoice\Mpdf\Image\Bmp
	 */
	private $bmp;

	/**
	 * @var \MemberPress\PdfInvoice\Mpdf\Image\Wmf
	 */
	private $wmf;
	...
	/**
	 * @var \MemberPress\PdfInvoice\Mpdf\DirectWrite
	 */
	private $directWrite;
	...
	/**
	 * @var \MemberPress\PdfInvoice\Mpdf\Fonts\FontCache
	 */
	private $fontCache;

	/**
	 * @var \MemberPress\PdfInvoice\Mpdf\Fonts\FontFileFinder
	 */
	private $fontFileFinder;

	/**
	 * @var \MemberPress\PdfInvoice\Mpdf\Tag
	 */
	private $tag;

	/**
	 * @var \MemberPress\PdfInvoice\Mpdf\Barcode
	 * @todo solve Tag dependency and make private
	 */
	public $barcode;

	/**
	 * @var \MemberPress\PdfInvoice\Mpdf\QrCode\QrCode
	 */
	private $qrcode;
	...
	/**
	 * @var \MemberPress\PdfInvoice\Mpdf\Color\ColorModeConverter
	 */
	private $colorModeConverter;

	/**
	 * @var \MemberPress\PdfInvoice\Mpdf\Color\ColorSpaceRestrictor
	 */
	private $colorSpaceRestrictor;

	/**
	 * @var \MemberPress\PdfInvoice\Mpdf\Hyphenator
	 */
	private $hyphenator;

	/**
	 * @var \MemberPress\PdfInvoice\Mpdf\Pdf\Protection
	 */
	private $protection;

	/**
	 * @var \MemberPress\PdfInvoice\Mpdf\Http\ClientInterface
	 */
	private $httpClient;

	/**
	 * @var \MemberPress\PdfInvoice\Mpdf\File\LocalContentLoaderInterface
	 */
	private $localContentLoader;

	/**
	 * @var \MemberPress\PdfInvoice\Mpdf\AssetFetcher
	 */
	...
	/**
	 * @var \MemberPress\PdfInvoice\Mpdf\Language\ScriptToLanguageInterface
	 */
	private $scriptToLanguage;

	/**
	 * @var \MemberPress\PdfInvoice\Mpdf\Writer\BaseWriter
	 */
	private $writer;

	/**
	 * @var \MemberPress\PdfInvoice\Mpdf\Writer\FontWriter
	 */
	private $fontWriter;

	/**
	 * @var \MemberPress\PdfInvoice\Mpdf\Writer\MetadataWriter
	 */
	private $metadataWriter;

	/**
	 * @var \MemberPress\PdfInvoice\Mpdf\Writer\ImageWriter
	 */
	private $imageWriter;

	/**
	 * @var \MemberPress\PdfInvoice\Mpdf\Writer\FormWriter
	 */
	private $formWriter;

	/**
	 * @var \MemberPress\PdfInvoice\Mpdf\Writer\PageWriter
	 */
	private $pageWriter;

	/**
	 * @var \MemberPress\PdfInvoice\Mpdf\Writer\BookmarkWriter
	 */
	private $bookmarkWriter;

	/**
	 * @var \MemberPress\PdfInvoice\Mpdf\Writer\OptionalContentWriter
	 */
	private $optionalContentWriter;

	/**
	 * @var \MemberPress\PdfInvoice\Mpdf\Writer\ColorWriter
	 */
	private $colorWriter;

	/**
	 * @var \MemberPress\PdfInvoice\Mpdf\Writer\BackgroundWriter
	 */
	private $backgroundWriter;

	/**
	 * @var \MemberPress\PdfInvoice\Mpdf\Writer\JavaScriptWriter
	 */
	private $javaScriptWriter;

	/**
	 * @var \MemberPress\PdfInvoice\Mpdf\Writer\ResourceWriter
	 */
	...
	/**
	 * @var \MemberPress\PdfInvoice\Mpdf\Container\ContainerInterface
	 */
	private $container;

	/**
	 * @param mixed[] $config
	 * @param \MemberPress\PdfInvoice\Mpdf\Container\ContainerInterface|null $container Experimental container to override internal services
	 */
	public function __construct(array $config = [], $container = null)
	{
		$this->_dochecks();

		assert(!$container || $container instanceof \MemberPress\PdfInvoice\Mpdf\Container\ContainerInterface);
		...
		if (file_exists($this->defaultCssFile)) {
			...
			throw new \MemberPress\PdfInvoice\Mpdf\MpdfException(sprintf('Unable to read default CSS file "%s"', $this->defaultCssFile));
		}
	...
	function _setPageSize($format, &$orientation)
	{
		if (is_string($format)) {
			...
			if (!$format[0] || !$format[1]) {
				throw new \MemberPress\PdfInvoice\Mpdf\MpdfException('Invalid page format: ' . $format[0] . ' ' . $format[1]);
			}
		...
		if ($orientation === 'p' || $orientation === 'portrait') {
			...
			throw new \MemberPress\PdfInvoice\Mpdf\MpdfException('Incorrect orientation: ' . $orientation);
		}
	...
	function SetDisplayMode($zoom, $layout = 'continuous')
	{
		...
		if (in_array($zoom, $allowedZoomModes, true) || is_numeric($zoom)) {
			...
			throw new \MemberPress\PdfInvoice\Mpdf\MpdfException('Incorrect zoom display mode: ' . $zoom);
		}
		...
		if (in_array($layout, $allowedLayoutModes, true)) {
			...
			throw new \MemberPress\PdfInvoice\Mpdf\MpdfException('Incorrect layout display mode: ' . $layout);
		}
	}
	...
	function SetVisibility($v)
	{
		...
		if ($v == 'printonly') {
			...
			throw new \MemberPress\PdfInvoice\Mpdf\MpdfException('Incorrect visibility: ' . $v);
		}
		$this->visibility = $v;
	}
	...
	function AddSpotColorsFromFile($file)
	{
		$colors = @file($file);
		if (!$colors) {
			throw new \MemberPress\PdfInvoice\Mpdf\MpdfException("Cannot load spot colors file - " . $file);
		}
	...
	function AddFont($family, $style = '')
	{
		...
		if ($this->usingCoreFont) {
			throw new \MemberPress\PdfInvoice\Mpdf\MpdfException("mPDF Error - problem with Font management");
		}
		...
		if (!isset($this->fontdata[$family][$stylekey]) || !$this->fontdata[$family][$stylekey]) {
			throw new \MemberPress\PdfInvoice\Mpdf\MpdfException(sprintf('Font "%s%s%s" is not supported', $family, $style ? ' - ' : '', $style));
		}
	...
	function SetFont($family, $style = '', $size = 0, $write = true, $forcewrite = false)
	{
		...
		if ($family == '') {
			if ($this->FontFamily) {
				...
				throw new \MemberPress\PdfInvoice\Mpdf\MpdfException("No font or default font set!");
			}
		}
		...
		if (($family == 'csymbol') || ($family == 'czapfdingbats') || ($family == 'ctimes') || ($family == 'ccourier') || ($family == 'chelvetica')) {
			if ($this->PDFA || $this->PDFX) {
				if ($family == 'csymbol' || $family == 'czapfdingbats') {
					throw new \MemberPress\PdfInvoice\Mpdf\MpdfException("Symbol and Zapfdingbats cannot be embedded in mPDF (required for PDFA1-b or PDFX/1-a).");
				}
		...
		if (!$this->onlyCoreFonts && !$this->usingCoreFont) {
			...
			if ($this->PDFA || $this->PDFX) {
				throw new \MemberPress\PdfInvoice\Mpdf\MpdfException('Core Adobe fonts cannot be embedded in mPDF (required for PDFA1-b or PDFX/1-a) - cannot use option to use core fonts.');
			}
			...
			if (!isset($this->fonts[$fontkey])) {
				...
				if (isset($this->CoreFonts[$fontkey])) {
					...
					if (!isset($cw)) {
						throw new \MemberPress\PdfInvoice\Mpdf\MpdfException(sprintf('Could not include font metric file "%s"', $file));
					}
					$i = count($this->fonts) + $this->extraFontSubsets + 1;
					...
					throw new \MemberPress\PdfInvoice\Mpdf\MpdfException(sprintf('Font %s not defined', $fontkey));
				}
			}
		...
		foreach ($this->objectbuffer as $ib => $objattr) {
			...
			if ($objattr['type'] === 'barcode') {
				...
				if (in_array($objattr['btype'], $barcodeTypes, true)) {
					...
					if (!class_exists('MemberPress\PdfInvoice\Mpdf\QrCode\QrCode') || !class_exists('MemberPress\PdfInvoice\Mpdf\QrCode\Output\Mpdf')) {
						throw new \MemberPress\PdfInvoice\Mpdf\MpdfException('MemberPress\PdfInvoice\Mpdf\QrCode package was not found. Install the package from Packagist with "composer require mpdf/qrcode"');
					}
	...
	function _SetTextRendering($mode)
	{
		if (!(($mode == 0) || ($mode == 1) || ($mode == 2))) {
			throw new \MemberPress\PdfInvoice\Mpdf\MpdfException("Text rendering mode should be 0, 1 or 2 (value : $mode)");
		}
		$tr = ($mode . ' Tr');
	...
	function Output($name = '', $dest = '')
	{
		...
		if ($this->debug && error_get_last()) {
			...
			if (($e['type'] < 2048 && $e['type'] != 8) || (intval($e['type']) & intval(ini_get("error_reporting")))) {
				throw new \MemberPress\PdfInvoice\Mpdf\MpdfException(
		...
		if (($this->PDFA || $this->PDFX) && $this->encrypted) {
			throw new \MemberPress\PdfInvoice\Mpdf\MpdfException('PDF/A1-b or PDF/X1-a does not permit encryption of documents.');
		}
		...
		if (count($this->PDFAXwarnings) && (($this->PDFA && !$this->PDFAauto) || ($this->PDFX && !$this->PDFXauto))) {
			...
			throw new \MemberPress\PdfInvoice\Mpdf\MpdfException('PDFA/PDFX warnings generated. See log for further details');
		}
		...
		switch ($dest) {
			...
			case Destination::INLINE:
				...
				if ($this->debug && !$this->allow_output_buffering && ob_get_contents()) {
					throw new \MemberPress\PdfInvoice\Mpdf\MpdfException('Output has already been sent from the script - PDF file generation aborted.');
				}
			...
			case Destination::DOWNLOAD:
				...
				if (headers_sent()) {
					throw new \MemberPress\PdfInvoice\Mpdf\MpdfException('Data has already been sent to output, unable to output PDF file');
				}
			...
			case Destination::FILE:
				...
				if (!$f) {
					throw new \MemberPress\PdfInvoice\Mpdf\MpdfException(sprintf('Unable to create output file %s', $name));
				}
			...
			default:
				throw new \MemberPress\PdfInvoice\Mpdf\MpdfException(sprintf('Incorrect output destination %s', $dest));
		}
	...
	function _dochecks()
	{
		...
		if (1.1 == 1) {
			throw new \MemberPress\PdfInvoice\Mpdf\MpdfException('Do not alter the locale before including mPDF');
		}
		...
		if (ini_get('mbstring.func_overload')) {
			throw new \MemberPress\PdfInvoice\Mpdf\MpdfException('Mpdf cannot function properly with mbstring.func_overload enabled');
		}

		if (!function_exists('mb_substr')) {
			throw new \MemberPress\PdfInvoice\Mpdf\MpdfException('mbstring extension must be loaded in order to run mPDF');
		}
		...
		if (!function_exists('mb_regex_encoding')) {
			...
			throw new \MemberPress\PdfInvoice\Mpdf\MpdfException('mbstring extension with mbregex support must be loaded in order to run mPDF.' . $mamp);
		}
	}
	...
	{
		if ($this->PDFA || $this->PDFX) {
			throw new \MemberPress\PdfInvoice\Mpdf\MpdfException('PDFA and PDFX do not permit transparency, so mPDF does not allow Watermarks!');
		}
	...
	function AddCIDFont($family, $style, $name, &$cw, $CMap, $registry, $desc)
	{
		...
		if (isset($this->fonts[$fontkey])) {
			throw new \MemberPress\PdfInvoice\Mpdf\MpdfException("Font already added: $family $style");
		}
		$i = count($this->fonts) + $this->extraFontSubsets + 1;
	...
	function AddCJKFont($family)
	{
		...
		if ($this->PDFA || $this->PDFX) {
			throw new \MemberPress\PdfInvoice\Mpdf\MpdfException("Adobe CJK fonts cannot be embedded in mPDF (required for PDFA1-b and PDFX/1-a).");
		}
		if ($family == 'big5') {
			$this->AddBig5Font();
		} elseif ($family == 'gb') {
	...
	function TableHeaderFooter($content = '', $tablestartpage = '', $tablestartcolumn = '', $horf = 'H', $level = 0, $firstSpread = true, $finalSpread = true)
	{
		if (($horf == 'H' || $horf == 'F') && !empty($content)) { // mPDF 5.7.2
			...
			if (empty($content[$firstrow])) {
				if ($this->debug) {
					throw new \MemberPress\PdfInvoice\Mpdf\MpdfException("<tfoot> must precede <tbody> in a table");
				} else {
					return;
				}
			}
	...
	{
		if ($txt instanceof \MemberPress\PdfInvoice\Mpdf\WatermarkText) {
	...
	{
		if ($src instanceof \MemberPress\PdfInvoice\Mpdf\WatermarkImage) {
	...
	function WriteHTML($html, $mode = HTMLParserMode::DEFAULT_MODE, $init = true, $close = true)
	{
		...
		if (is_scalar($html) === false) {
			if (!is_object($html) || ! method_exists($html, '__toString')) {
				throw new \MemberPress\PdfInvoice\Mpdf\MpdfException('WriteHTML() requires $html be an integer, float, string, boolean or an object with the __toString() magic method.');
			}
		}

		// Check the mode is valid
		if (in_array($mode, HTMLParserMode::getAllModes(), true) === false) {
			throw new \MemberPress\PdfInvoice\Mpdf\MpdfException('WriteHTML() requires $mode to be one of the modes defined in HTMLParserMode');
		}
	...
	function printbuffer($arrayaux, $blockstate = 0, $is_table = false, $table_draft = false, $cell_dir = '')
	{
		...
		for ($i = 0; $i < $array_size; $i++) {
			...
			if (($blockdir == 'rtl' || $this->biDirectional) && isset($arrayaux[$i][4]) && in_array($arrayaux[$i][4], ['ccourier', 'ctimes', 'chelvetica', 'csymbol', 'czapfdingbats'])) {
				throw new \MemberPress\PdfInvoice\Mpdf\MpdfException("You cannot use core fonts in a document which contains RTL text.");
			}
		}
		// mPDF 6
	...
	// mPDF 6
		...
		if (($toonarrow) && ($this->table_error_report)) {
			throw new \MemberPress\PdfInvoice\Mpdf\MpdfException("Word is too long to fit in table - " . $this->table_error_report_param);
		}
		if ($toonarrow) {
			return -$biggestword;
		} else {
		...
		if (!$this->is_utf8($html)) {
			...
			while (mb_convert_encoding(mb_convert_encoding($html, "UTF-32", "UTF-8"), "UTF-8", "UTF-32") !== $html) {
				...
				if ($error && $error['message'] === 'iconv(): Detected an illegal character in input string') {
					throw new \MemberPress\PdfInvoice\Mpdf\MpdfException('Invalid input characters. Did you set $mpdf->in_charset properly?');
				}
			...
			throw new \MemberPress\PdfInvoice\Mpdf\MpdfException("HTML contains invalid UTF-8 character(s). See log for further details");
		}
	...
	function purify_utf8_text($txt)
	{
		...
		if (!$this->is_utf8($txt)) {
			throw new \MemberPress\PdfInvoice\Mpdf\MpdfException("Text contains invalid UTF-8 character(s)");
		}
	...
	function WriteBarcode($code, $showtext = 1, $x = '', $y = '', $size = 1, $border = 0, $paddingL = 1, $paddingR = 1, $paddingT = 2, $paddingB = 2, $height = 1, $bgcol = false, $col = false, $btype = 'ISBN', $supplement = '0', $supplement_code = '', $k = 1)
	{
		...
		if ($arrcode === false) {
			throw new \MemberPress\PdfInvoice\Mpdf\MpdfException('Error in barcode string: ' . $codestr);
		}
		...
		if ($supplement) { // EAN-2 or -5 Supplement
			...
			if ($supparrcode === false) {
				throw new \MemberPress\PdfInvoice\Mpdf\MpdfException('Error in barcode string (supplement): ' . $codestr . ' ' . $supplement_code);
			}

			if (strlen($supplement_code) != $supplement) {
				throw new \MemberPress\PdfInvoice\Mpdf\MpdfException('Barcode supplement incorrect: ' . $supplement_code);
			}
	...
	function transformScale($s_x, $s_y, $x = '', $y = '', $returnstring = false)
	{
		...
		if (($s_x == 0) or ( $s_y == 0)) {
			throw new \MemberPress\PdfInvoice\Mpdf\MpdfException('Please do not use values equal to zero for scaling');
		}
	...
	function AdjustHTML($html, $tabSpaces = 8)
	{
		...
		if (0 >= (int) $limit) {
			throw new \MemberPress\PdfInvoice\Mpdf\MpdfException(sprintf(
	...
	function OverWrite($file_in, $search, $replacement, $dest = Destination::DOWNLOAD, $file_out = "mpdf")
	{
		...
		switch ($dest) {
			...
			case Destination::FILE:
				...
				if (!$f) {
					throw new \MemberPress\PdfInvoice\Mpdf\MpdfException('Unable to create output file: ' . $file_out);
				}
