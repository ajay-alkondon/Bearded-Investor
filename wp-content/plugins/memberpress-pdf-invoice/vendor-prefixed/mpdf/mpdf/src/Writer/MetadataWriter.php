...
namespace MemberPress\PdfInvoice\Mpdf\Writer;
...
use MemberPress\PdfInvoice\Mpdf\Utils\PdfDate;

use MemberPress\PdfInvoice\Psr\Log\LoggerInterface;

class MetadataWriter implements \MemberPress\PdfInvoice\Psr\Log\LoggerAwareInterface
{
	  ...
	  <rdf:li>' . $this->mpdf->author . '</rdf:li>
		...
		elseif ($this->mpdf->PDFA) {
			...
			if (strpos($this->mpdf->PDFAversion, '-') === false) {
				throw new \MemberPress\PdfInvoice\Mpdf\MpdfException(sprintf('PDFA version (%s) is not valid. (Use: 1-B, 3-B, etc.)', $this->mpdf->PDFAversion));
			}
	...
	public function writeOutputIntent() // _putoutputintent
	{
		...
		if ($this->mpdf->ICCProfile) {
			if (!file_exists($this->mpdf->ICCProfile)) {
				throw new \MemberPress\PdfInvoice\Mpdf\MpdfException(sprintf('Unable to find ICC profile "%s"', $this->mpdf->ICCProfile));
			}
	...
	public function writeAssociatedFiles() // _putAssociatedFiles
	{
		if (!function_exists('gzcompress')) {
			throw new \MemberPress\PdfInvoice\Mpdf\MpdfException('ext-zlib is required for compression of associated files');
		}
		...
		foreach ($this->mpdf->associatedFiles as $k => $file) {
			...
			if (!$fileContent) {
				throw new \MemberPress\PdfInvoice\Mpdf\MpdfException(sprintf('Cannot access associated file - %s', $file['path']));
			}
				...
				if (isset($this->mpdf->PageAnnots[$n])) {
					...
					foreach ($this->mpdf->PageAnnots[$n] as $key => $pl) {
						...
						if ($fileAttachment) {
							...
							if (!$file) {
								throw new \MemberPress\PdfInvoice\Mpdf\MpdfException('mPDF Error: Cannot access file attachment - ' . $pl['opt']['file']);
							}
