jQuery(document).ready(function ($) {
  ...
  $('a.mepr_resend_txn_invoice_email').click(function() {
    var txn_id = jQuery(this).attr('data-value');

    $('tr#record_' + txn_id + ' .mepr_loader').show();

    var data = {
      action: 'mepr_resend_txn_invoice_email',
      nonce: MeprPDFInvoice.nonce,
      id: txn_id
    };

    $.post(MeprPDFInvoice.ajax_url, data, function(response) {
      var trimmed_data = response.replace(/^\s+|\s+$/g, ''); //Trim whitespace

      jQuery('tr#record_' + txn_id + ' .mepr_loader').hide();
