                ...
                if (isset($expires['expires_at'])) {
                    ...
                    if ($expires_at && $expires_at < time()) {
                        ...
                        if (!empty($licenses) && is_array($licenses)) {
                              $highest_edition_index = 0;
                              $highest_license       = null;

                            foreach ($licenses as $license) {
                                $edition = MeprUtils::get_edition($license['product_slug']);

                                if (is_array($edition) && $edition['priority'] > $highest_edition_index) {
                                          $highest_edition_index = $edition['priority'];
    ...
    public static function activate_stripe_payment_method()
    {
        ...
        try {
            if ($payment_method_type === 'apple_pay') {
                $root_dir                     = isset($_SERVER['DOCUMENT_ROOT']) ? sanitize_text_field(wp_unslash($_SERVER['DOCUMENT_ROOT'])) : ABSPATH;
                ...
                if (!file_exists($full_path)) {
                    if (!is_dir($well_known_dir)) {
                        ...
                        $filesystem->mkdir($well_known_dir);
                        $filesystem->chmod($well_known_dir, 0755);
