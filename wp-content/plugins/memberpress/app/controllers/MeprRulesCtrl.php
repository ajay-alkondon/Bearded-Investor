...
class MeprRulesCtrl extends MeprCptCtrl
{
    ...
    public static function save_postdata($post_id)
    {
        ...
        if (!empty($post) && $post->post_type === MeprRule::$cpt) {
            ...
            $rule->mepr_content          = (('partial' !== $_POST[MeprRule::$mepr_type_str] && isset($_POST[MeprRule::$mepr_content_str])) ? sanitize_text_field(wp_unslash($_POST[MeprRule::$mepr_content_str])) : '');
            ...
            if (isset($_POST['mepr_access_row']) && !empty($_POST['mepr_access_row'])) {
                foreach (array_map('sanitize_text_field', wp_unslash($_POST['mepr_access_row']['type'])) as $index => $access_type) {
                    $rule_access_condition                   = new MeprRuleAccessCondition(sanitize_text_field(wp_unslash($_POST['mepr_access_row']['rule_access_condition_id'][$index])));
