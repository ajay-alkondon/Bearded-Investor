...
<div class="mp_wrapper mepr-payments-wrapper table-responsive">
  ...
  <tbody>
    ...
    <td data-label="<?php echo esc_attr_x('Invoice', 'ui', 'memberpress'); ?>">
      ...
      <p class="sub"><?php echo MeprHooks::apply_filters('mepr_account_payment_product_name', $prd->post_title, $txn); ?></p>
    </td>
    ...
    echo '<div class="btn mepr-pro-account-table__badge --is-' . MeprAppHelper::pro_template_txn_status($txn) . '">' . MeprAppHelper::human_readable_status($txn->status, 'transaction') . '</div>';
    ...
    <td class="mepr-pro-account-table__col-actions">
        <button type="button" id="mepr-action-<?php echo $payment->id; ?>" class="mepr-tooltip-trigger" aria-haspopup="true" aria-controls="mepr-tooltip-content-<?php echo $payment->id; ?>" aria-expanded="false">
      ...
      <div class="mepr-tooltip-content" id="mepr-tooltip-content-<?php echo $payment->id; ?>" aria-labelledby="mepr-action-<?php echo $payment->id; ?>" role="menu">
            <?php
            echo trim(wp_kses(
                $row_actions,
                [
