            ...
            if ($txn instanceof MeprTransaction && $txn->is_sub_account()) {
                ...
                if (
                    ...
                    if ($prd->is_one_time_payment() && $prd->is_renewable() && $prd->is_renewal()) {
                        ?>
                <a href="<?php echo $prd->url(); ?>" class="mepr-account-row-action mepr-account-renew"><?php echo esc_html_x('Renew', 'ui', 'memberpress'); ?></a>
                        <?php
                    }
          ...
          <tr x-data="{open:false}">
            <td data-label="<?php echo esc_attr_x('Membership', 'ui', 'memberpress'); ?>">
              <div class="mepr-pro-account-table__product">
                <?php if (isset($prd->access_url) && !empty($prd->access_url)) : ?>
                  <a href="<?php echo stripslashes($prd->access_url); ?>"><?php echo MeprHooks::apply_filters('mepr_account_subscr_product_name', $prd->post_title, $txn); ?></a>
                <?php else : ?>
                    <?php echo MeprHooks::apply_filters('mepr_account_subscr_product_name', $prd->post_title, $txn); ?>
                <?php endif; ?>
              </div>
            ...
            <td data-label="<?php echo esc_attr_x('Status', 'ui', 'memberpress'); ?>">
              <?php
                ...
                echo '<div class="btn mepr-pro-account-table__badge --is-' . $sub_status . '">' .  esc_html(MeprAppHelper::status_human_readable($sub_status)) . '</div>';
                ?>
            </td>
            ...
            <td class="mepr-pro-account-table__col-actions" data-label="<?php echo esc_attr_x('Actions', 'ui', 'memberpress'); ?>">
              <?php if (trim($row_actions)) { ?>
                <button type="button" id="mepr-action-<?php echo $s->id; ?>" class="mepr-tooltip-trigger" aria-haspopup="true" aria-controls="mepr-tooltip-content-<?php echo $s->id; ?>" aria-expanded="false">
                ...
                <div
                  ...
                  id="mepr-tooltip-content-<?php echo $s->id; ?>"
                  aria-labelledby="mepr-action-<?php echo $s->id; ?>"
                  role="menu"
                >
