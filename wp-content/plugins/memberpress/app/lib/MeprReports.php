...
class MeprReports
{
    ...
    public static function get_monthly_data($type, $month, $year, $product, $q = [])
    {
        ...
        for ($i = 1; $i <= $days_in_month; $i++) {
            ...
            $results[$i] = $wpdb->get_row($wpdb->prepare($base_query, ...$params)); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
        }
    ...
    ";
        ...
        $res = $wpdb->get_col($q); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
    ...
    public static function subscription_stats($created_since = false)
    {
      ...
      ",
        ...
        $stats = $wpdb->get_row($q, ARRAY_A); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
      ...
      ",
        ...
        $stats = array_merge($stats, $wpdb->get_row($q, ARRAY_A)); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
    ...
    public static function transaction_stats($created_since = false)
    {
      ...
      ",
        ...
        return $wpdb->get_row($q); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
    }
    ...
    public static function gateway_revenue_stats($payment_method_id)
    {
      ...
      ",
        ...
        $rev_stats = $wpdb->get_row($q, ARRAY_A); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
      ...
      ",
        ...
        $ref_stats = $wpdb->get_row($q, ARRAY_A); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
    ...
    public static function get_monthly_dataset($type, $month, $year, $product, $q = [])
    {
        ...
        foreach ($queries as $type => $sql) {
            ...
            $resultset = $wpdb->get_results($sql); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
    ...
    public static function get_revenue_dataset($month, $year, $product = null)
    {
        ...
        return self::format_mepr_dataset($wpdb->get_results($query)); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
    }
