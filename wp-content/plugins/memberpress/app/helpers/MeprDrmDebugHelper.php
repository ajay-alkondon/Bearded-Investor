...
class MeprDrmDebugHelper
{
    ...
    private static function get_app_fee_subscription_counts()
    {
        ...
        return [
            'applied' => [
                'count' => number_format((int) $wpdb->get_var($active_applied_sql)), // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
                'copy_sql' => $copy_applied_sql,
            ],
            'revoked' => [
                'count' => number_format((int) $wpdb->get_var($active_revoked_sql)), // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
                'copy_sql' => $copy_revoked_sql,
            ],
            'all_applied' => [
                'count' => number_format((int) $wpdb->get_var($all_applied_sql)), // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
                'copy_sql' => '',
            ],
            'all_revoked' => [
                'count' => number_format((int) $wpdb->get_var($all_revoked_sql)), // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
                'copy_sql' => '',
            ],
        ];
    }
