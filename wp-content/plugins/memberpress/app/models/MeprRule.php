...
class MeprRule extends MeprCptModel
{
    ...
    public static function get_single_array($type)
    {
        ...
        $lookup = $wpdb->get_results($wpdb->prepare("SELECT ID, post_title FROM {$wpdb->posts} WHERE post_type = %s", $type), OBJECT_K);
    ...
    public static function search_terms($tax, $search = '', $limit = 25)
    {
        ...
        return array_map(
            ...
            $wpdb->get_results($prepared_query) // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
        );
    }
