 ...
 */
class MeprSquarePaymentsGateway extends MeprBaseRealAjaxGateway
{
    ...
    public function enqueue_payment_form_scripts()
    {
        ...
        wp_localize_script(
            'mepr-square',
            'MeprSquarePaymentsGateway',
            ['l10n_print_after' => 'MeprSquarePaymentsGateway = ' . wp_json_encode($l10n)]
        );
    }
    ...
    public function api_request(
            ...
            if (!in_array($environment, ['production', 'sandbox'], true)) {
                throw new InvalidArgumentException(
                    sprintf(
                        ...
                        esc_html__('The environment must be either "production" or "sandbox", but "%s" was given.', 'memberpress'),
        ...
        if (isset($json['errors'][0]['code'], $json['errors'][0]['detail'])) {
            throw new MeprRemoteException(sprintf(
                // Translators: %1$s: error detail, %2$s: error code.
                esc_html__('%1$s (%2$s)', 'memberpress'),
                esc_html($json['errors'][0]['detail']),
                esc_html($json['errors'][0]['code'])
            ));
        } else {
            throw new MeprRemoteException(esc_html__('There was an issue with the payment processor. Try again later.', 'memberpress'));
        }
    }
    ...
    public static function get_cadence(string $period_type, int $period): string
    {
        ...
        if ($cadence === '') {
            throw new MeprGatewayException(esc_html__('Unsupported subscription cadence', 'memberpress'));
        }
    ...
    {
        if (empty($invoice['subscription_id']) || empty($invoice['status']) || strtoupper($invoice['status']) !== 'UNPAID') {
            return;
        }
    ...
    protected function handle_subscription_updated_webhook(array $subscription): void
    {
        ...
        if (in_array($subscription['status'], ['CANCELED', 'DEACTIVATED'], true)) {
    ...
    public function service_listener()
    {
        ...
        if ($event === 'update-credentials') {
            ...
            if (empty($site_uuid) || (string) $auth_site_uuid !== $site_uuid) {
    ...
    public function disconnect(string $environment, string $type): void
    {
        ...
        if (wp_remote_retrieve_response_code($response) !== 204) {
            throw new Exception(esc_html__('Invalid response.', 'memberpress'));
        }
    }
    // phpcs:enable Squiz.Commenting.FunctionCommentThrowTag.WrongNumber
    ...
    protected function record_cancel_sub(MeprSubscription $sub): void
    {
        ...
        if (!isset($_REQUEST['silent']) || !sanitize_text_field(wp_unslash($_REQUEST['silent']))) {
    ...
    public function process_update_account_form_ajax(MeprSubscription $sub)
    {
        ...
        if (empty($source_id) || empty($idempotency_key)) {
            throw new MeprGatewayException(esc_html__('Bad request', 'memberpress'));
        }
    ...
    public function process_resume_subscription($subscription_id)
    {
        ...
        $this->api_request(
            'POST',
            "/v2/subscriptions/$sub->subscr_id/resume",
            MeprHooks::apply_filters(
                'mepr_square_resume_subscription_data',
                [
    ...
    /**
     * Get the renewal base date for a given subscription.
     *
     * This date is used to calculate expiration dates.
     *
     * @param MeprSubscription $sub The subscription.
     *
     * @return string The renewal base date.
     */
    public function get_renewal_base_date(MeprSubscription $sub): string
    {
        global $wpdb;
        $db = MeprDb::fetch();

        $resume_date = $wpdb->get_var(
            $wpdb->prepare(
                // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
                "SELECT e.created_at FROM $db->events AS e WHERE e.event = %s AND e.evt_id_type = %s AND e.evt_id = %d ORDER BY e.created_at DESC LIMIT 1",
        ...
        return parent::get_renewal_base_date($sub);
    }
